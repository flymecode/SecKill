...
#### 解决连接消耗
我们使用keepalive解决nginx和server连接的问题，使用数据库连接池我们解决数据库建立连接的数据消耗，我设置tomcat初始等待队列长度和
线程数量减少建立线程的时间消耗。使用nginx反向代理降低单机瓶颈。
...
秒杀验证逻辑和秒杀下单的接口强关联，代码冗余，秒杀逻辑复杂，对交易系统产生无关联的负载


秒杀令牌

秒杀接口需要依靠令牌才能进入，秒杀的令牌由秒杀活动模块来负责生成。秒杀前需要获得令牌才可以秒杀。
...
### 库存流水
在我们开始创建一个订单的时候，我们为这个订单创建一个流水记录，我们在以后的流程里
根据这个流水记录来控制我们逻辑，并且根据流水来进行回滚操作。

## 秒杀令牌
用户的校验在成功之后返回一个令牌，用户根据令牌来进行秒杀
秒杀令牌的生成数量的限制
秒杀大闸进行令牌的数量的控制
...
## 泄洪
本地队列，使用线程池，没有网络请求的消耗。
外部队列，将队列设置到外部之后做成集中化的管理。但是有单点的瓶颈。

## 限流的目的
活着总比死了要好，线上很多流量，我保证一部分用户用
